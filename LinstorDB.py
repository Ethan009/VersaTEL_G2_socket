...
class LINSTORDB():
    ...
    def __init__(self):
        self.start=nowTimes()
        print('linstor_start_conn_DB_time:',self.start)
        self.con = sqlite3.connect(':memory:',check_same_thread=False)
        self.cur = self.con.cursor()
        self.drop_tb()
        print('GUI_client_get_data:',nowTimes())
        b = vst.conn(b'python3 vtel.py stor gui -db')
        #print('---------linstor(data transfer completed)')
        #print(b)
        self.cur.executescript(b)
        self.end=nowTimes()
        #print('LinstorDB.py:end-start=',self.end-self.start)
